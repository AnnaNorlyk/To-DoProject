...
public class TodoListService : ITodoListService
{
    ...
    public TodoListService(TodoContext ctx, ILogger<TodoListService> logger)
    {
        _ctx = ctx;
        _logger = logger;
    }
    ...
    public async Task<bool> DeleteListAsync(int listId)
    {
        ...
        if (list is null)
        {
            _logger.LogWarning("List with ID {Id} not found for deletion", listId);
            return false;
        }
        ...
        _logger.LogInformation("Deleted list with ID {Id}", listId);
        return true;
    }

    public async Task<List<TodoListDto>> GetAllListsAsync()
    {
        _logger.LogInformation("Fetching all lists from database");
